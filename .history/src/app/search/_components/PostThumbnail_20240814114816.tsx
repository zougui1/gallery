...
export const PostThumbnail = ({ post }: PostThumbnailProps) => {
  return (
    <figure
      className={cn(
        'inline-flex flex-col justify-center items-center gap-1 relative max-w-max',
    ...
    >
      <Link
        href={`/posts/${post._id}`}
        className={cn(
          'relative shadow rounded-md transition-all max-w-full block w-auto max-w-full',
      ...
      >
        <picture>
          ...
          <img
            ...
            className="block width-auto w-full h-48 rounded-md"
          />
        </picture>
      ...
      <figcaption className="w-full table-caption caption-bottom" style={{ wordWrap: 'break-word' }}>
