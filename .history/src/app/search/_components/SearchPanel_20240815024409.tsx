  ...
  return (
    <MainLayout.SidePanel className="max-w-64 w-full flex flex-col gap-6">
      ...
      <div className="flex flex-col">
        ...
        <div className="flex gap-8">
          <label>
            <input type="checkbox" checked={types.includes(PostType.)} onChange={() => toggleType(PostType.sfw)} />
            <span className="ml-2">{PostType.sfw.toUpperCase()}</span>
          </label>

          <label>
            <input type="checkbox" checked={types.includes(PostType.nsfw)} onChange={() => toggleType(PostType.nsfw)} />
            <span className="ml-2">{PostType.nsfw.toUpperCase()}</span>
          </label>
        </div>
      </div>
