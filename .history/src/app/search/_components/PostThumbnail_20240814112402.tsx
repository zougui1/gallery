...
export const PostThumbnail = ({ post }: PostThumbnailProps) => {
  return (
    <figure className="flex flex-col justify-center items-center gap-1 relative max-w-max inline-block">
      <Link
        href={`/posts/${post._id}`}
        className={cn(
      ...
      <figcaption className="max-w-full">
        <Link href={`/api/media/${post._id}`}>
          <Typography.Paragraph
            className="text-sm font-semibold transition-all text-blue-100 hover:text-foreground"
          >
            {post.title}
          </Typography.Paragraph>
        </Link>
