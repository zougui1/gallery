...
export const SearchResult = ({ searchParams, hasMore }: SearchResultProps) => {
  ...
  return (
    <div className="flex justify-center items-center gap-4">
      <ButtonLink.Internal disabled={searchParams.page <= 1} href={`/search?${prevParams.toString()}`}>Previous</ButtonLink.Internal>

      <Typography.Span>Search result page #{searchParams.page}</Typography.Span>

      <ButtonLink.Internal disabled={!hasMore} href={`/search?${nextParams.toString()}`}>
          Next
      </ButtonLink.Internal>
    </div>
  );
