...
const getColumns = ({
  ...
  onNewAlt: (data: SubmissionUploadSchema['asAlt']) => void;
  onEditAlt: (data: SubmissionUploadSchema['asAlt']) => void;
}) => [
  {
    accessorKey: 'label',
    header: 'Label',
...
] satisfies ColumnDef<SubmissionUploadSchema['asAlt']>[];

export const AltTable = ({ alts, onAltsChange, defaultValue }: AltTableProps) => {
  const onNewAlt = (data: SubmissionUploadSchema['asAlt']): void => {
    onAltsChange([...alts, data]);
  }

  const onEditAlt = (data: SubmissionUploadSchema['asAlt']): void => {
...
export interface AltTableProps {
  alts: SubmissionUploadSchema['asAlt'][];
  onAltsChange: (alts: SubmissionUploadSchema['asAlt'][]) => void;
  defaultValue: SubmissionUploadSchema['asEither'];
}
