...
export const AltFormDialog = ({ children, onSubmit, defaultValue }: AltFormDialogProps) => {
  const form = useForm<Omit<SubmissionUploadSchema['asAlt'], 'createdAt'>>({
    resolver: zodResolver(submissionUploadSchema.asAlt.omit({ createdAt: true })),
    defaultValues: {
      alt: {
...
export interface AltFormDialogProps {
  ...
  defaultValue?: Omit<SubmissionUploadSchema['asEither'], 'createdAt'>;
}
