...
const downloadFile = async (url: string, filePath: string): Promise<void> => {
  ...
  //const fileType = await fileTypeStream(stream.pipeline(response.data, writeStream));
  const fileStream = await fileTypeStream(response.data);
  ...
  console.log('fileType:', fileStream.fileType)
}
