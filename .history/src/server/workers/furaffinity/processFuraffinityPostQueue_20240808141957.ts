...
import { scanForSimilarities } from './scanForSimilarities';
...
export const processFuraffinityPostQueue = async (postQueue: WithId<FuraffinityUrlUploadPostQueueSchema>) => {
  ...
  const [scanError] = await tryStep(scanForSimilarities, postQueue, 'An error occured while scanning the submissions')(
    postQueue,
    newPost,
  );
