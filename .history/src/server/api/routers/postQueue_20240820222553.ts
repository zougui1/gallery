...
const createSubmission = async (input: z.infer<typeof submissionUploadSchema>): Promise<void> => {
  ...
  // TODO handle this check more gracefully for alts/comics/stories
  if (normalizedUrl) {
...
export const postQueueRouter = createTRPCRouter({
  create: publicProcedure
    .input(z.union([submissionUploadSchema.transform(value => [value]), z.array(submissionUploadSchema)]))
  ...
  createStorySeries: publicProcedure
    .input(z.object({
      name: z.string().min(1),
      units: z.array(submissionUploadSchema),
    }))
    .mutation(async ({ input }) => {
      console.log(input);
